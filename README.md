...
App 默认开启打卡提醒，每天 10:00 和 22:00 各提醒一次。如果当天已经打卡，对应时间点不会再弹提醒（例如你 22:00 前已经打过卡，晚上那条提醒就会自动取消）。点击提醒会打开 App 并自动完成当天打卡。

可在首页的「打卡提醒」开关随时关闭或重新开启。

首次开启时，Android 13 及以上系统会弹出通知授权请求，请选择允许；如果拒绝，开关会自动回到关闭状态。

### 小米 15 / HyperOS 重要设置

小米（MIUI / HyperOS）默认会限制后台定时任务，**不做以下设置，提醒很可能不会准时弹出甚至完全收不到**。安装后请手动开启：

1. 进入「设置 → 应用设置 → 应用管理 → 每日打卡」。
2. 打开「自启动」。
3. 进入「省电策略」，选择「无限制」。
4. 确认「通知管理」中允许该 App 发送通知。

这些是系统层面的限制，App 代码无法绕过。如果某天没收到提醒，优先检查以上三项。提醒只是“提示去打开 App”，不会在后台替你自动打卡。
...
V2.5 增加了本地打卡提醒：每天 10:00 和 22:00 提醒打卡，当天已打卡则自动跳过，可在首页一键开关。提醒由 WorkManager 调度，手机重启后自动恢复，仍然不联网、不在后台自动打卡。V2.4 增加了当天首次打卡成功动画。V2 系列还包含新版绿色视觉系统、月历、列表、统计信息和自适应应用图标。
...
开启打卡提醒后，系统会在 10:00 和 22:00 用 WorkManager 触发一次轻量检查来决定是否弹提醒，除此之外没有常驻后台进程，也不会联网或在后台自动打卡。关闭提醒开关后则完全没有后台任务。
